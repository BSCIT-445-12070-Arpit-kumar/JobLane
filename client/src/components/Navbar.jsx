              ...
              {!isMobile &&  <ul className='sm:flex  justify-center items-center gap-24 pt-4 pb-3 font-semibold text-xl'>
                    ...
                    {isLogin ? (
                        <Menu shadow="md" width={200}>
                            <Menu.Target>
                                <Avatar className='cursor-pointer fixed right-32' radius="xl" src={me?.avatar?.url || "https://via.placeholder.com/150"} alt="it's me" />
                            </Menu.Target>

                            <Menu.Dropdown>
                                <Link to="/profile"><Menu.Item icon={<FaUserCircle size={14} />}>My Profile</Menu.Item></Link>
                                {role === "admin" && <Link to="/admin/dashboard"><Menu.Item icon={<MdOutlineDashboard size={14} />}>Dashboard</Menu.Item></Link>}
                                {role === "recruiter" && <Link to="/recruiter/dashboard"><Menu.Item icon={<MdOutlineDashboard size={14} />}>My Dashboard</Menu.Item></Link>}
                                {role === "recruiter" && <Link to="/admin/postJob"><Menu.Item icon={<MdOutlineBusinessCenter size={14} />}>Post Job</Menu.Item></Link>}
                                {role !== "recruiter" && <Link to="/applied"><Menu.Item icon={<MdDoneAll size={14} />}>Applied Jobs</Menu.Item></Link>}
                                {role !== "recruiter" && <Link to="/saved"><Menu.Item icon={<FaSave size={14} />}>Saved Jobs</Menu.Item></Link>}
                                <Menu.Divider />
                        ...
                        <span className='fixed right-24 flex gap-3'>
                            <Link className='cursor-pointer text-sm px-3 py-1 rounded-xl blueCol' to="/login/choice">Login</Link>
                    ...
                    <div className='flex justify-center items-center'>
                        <div className='pr-12'>
                            {isLogin ? (
                                <Menu shadow="md" width={200}>
                                    <Menu.Target>
                                        <Avatar size={28} className='cursor-pointer' radius="xl" src={me?.avatar?.url || "https://via.placeholder.com/150"} alt="it's me" />
                                    </Menu.Target>
