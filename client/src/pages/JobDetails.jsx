...
export const JobDetails = () => {
  ...
  const hasApplied = me.appliedJobs && me.appliedJobs.includes(jobDetails._id)
              ...
              <div className='md:px-12 pl-4'>
                ...
                <div>
                  <ul className='flex flex-col gap-3'>
                    <li className='flex items-center gap-3'>Posted By: <div>{jobDetails.postedBy?.name || "Unknown recruiter"}</div></li>
                    ...
                    <li className='flex items-center gap-3'>Salary: <div className='flex items-center gap-1' ><BiRupee />  <span>{formatSalaryDisplay(jobDetails.salary)}</span></div></li>
              ...
              <div className='md:px-12 pl-4 flex gap-8 pb-32 pt-6 '>
                <button 
                  onClick={()=>{
                    ...
                    if (isRecruiter) {
                      toast.info("Only job seekers can apply for jobs.")
                      return
                    }

                    isLogin ? 
                    
                    hasApplied ? toast.error("You are already applied !") :
                ...
                className={`md:text-lg text-sm font-bold px-10 py-1.5 flex items-center gap-1 ${
                  isJobClosed || isRecruiter
                    ...
                    : "hover:bg-green-600 bg-green-800"
                }`}> <BsSend /> {isJobClosed ? "Closed" : isRecruiter ? "Applicants Only" : hasApplied ? "Applied" : "Apply"}</button>
